...
class InteractionEffects {
  ...
  static Future<void> load() async {
    ...
    enabled.value = prefs.getBool('interaction_effects') ?? true;
  }

  static Future<void> setEnabled(bool value) async {
    enabled.value = value;
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('interaction_effects', value);
  }

  static bool animate(BuildContext context) =>
      enabled.value && !MediaQuery.disableAnimationsOf(context);
  static void pulse() {
    if (enabled.value) HapticFeedback.selectionClick().catchError((_) {});
  }

  static Future<void> settings(BuildContext context) => showDialog<void>(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('مؤثرات التفاعل'),
          content: ValueListenableBuilder<bool>(
            valueListenable: enabled,
            builder: (context, value, _) => SwitchListTile(
              title: const Text('النبضات والحركات الخفيفة'),
              subtitle:
                  const Text('يبقى توضيح حالة الأزرار ظاهرًا عند الإيقاف'),
              value: value,
              onChanged: (value) {
                pulse();
                setEnabled(value);
          ...
          actions: [
            FeedbackTap(
                ...
                child: const Padding(
                    padding: EdgeInsets.all(12), child: Text('تم')))
          ],
        ),
      );
}
