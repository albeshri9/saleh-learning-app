...
class LessonController
  ...
  Future<LessonRunState> build(String arg) async {
    ...
    for (final sceneId in _completedScenes) {
      if (!sceneId.startsWith('welcome') &&
          !sceneId.startsWith('nasheed') &&
          !sceneId.startsWith('success')) {
        await _journal.award('lesson:$arg:$sceneId', 10);
      }
    }
  ...
  Future<void> completeScene({bool skipped = false}) async {
    ...
    try {
      if (!skipped &&
        ...
        await _journal.award('lesson:${s.lesson.id}:${s.currentScene.id}', 10);
  ...
  Future<void> _finish(LessonRunState s) async {
    await _journal.award('lesson:${s.lesson.id}:complete', 20);
    ref.invalidate(journalDataProvider);
