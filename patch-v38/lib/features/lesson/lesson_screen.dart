...
class _CompactLesson extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      LayoutBuilder(builder: (context, bounds) {
        final writing = scene.type == SceneType.freeWriting ||
            scene.type == SceneType.guidedWriting;
        final characterWidth =
            writing ? 82.0 : (bounds.maxWidth * .18).clamp(100.0, 210.0);
        return Padding(
...
class _StepsRail extends StatelessWidget {
  const _StepsRail(
      {required this.scenes, required this.currentIndex, this.compact = false});
  final List<Scene> scenes;
  final int currentIndex;
  final bool compact;
  @override
  Widget build(BuildContext context) {
    final active = lessonMilestone(scenes[currentIndex].type);
    return SizedBox(
            ...
            if (i > 0)
              const Padding(
                  ...
                  child: Icon(Icons.west_rounded,
                      color: Color(0xFF9E8BB6), size: 16)),
            Expanded(
              child: Semantics(
                  label: '${[
                    'أسمع',
                  ...
                  child: Container(
                      ...
                      child: Row(
                          ...
                          children: [
                            ToyIcon(
                                [Toy.book, Toy.mic, Toy.pencil, Toy.puzzle][i],
                                size: compact ? 36 : 48),
                            const SizedBox(width: 5),
                            Flexible(
                                child: Text(['أسمع', 'أنطق', 'أكتب', 'ألعب'][i],
                                    style: const TextStyle(
                                        fontSize: 16,
                                        color: profileInk,
                                        fontWeight: FontWeight.bold))),
