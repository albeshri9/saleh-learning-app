...
void main() {
  ...
  testWidgets(
    ...
    expect(
        tester.getSize(find.byType(FreeWritingCanvas)).width, greaterThan(650));
    for (var i = 0; i < 3; i++) {
      expect(
          tester.getCenter(find.byKey(ValueKey('milestone-$i'))).dx,
          greaterThan(
              tester.getCenter(find.byKey(ValueKey('milestone-${i + 1}'))).dx));
    }
    expect(tester.takeException(), isNull);
    await tester.pumpWidget(const SizedBox());
  });
