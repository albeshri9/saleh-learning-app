...
class _WorldScreenState extends ConsumerState<WorldScreen> {
  ...
  void _lesson({int? scene}) {
    ...
    unawaited(context
        .push<void>('/lesson/alif${scene == null ? '' : '?scene=$scene'}')
        .then((_) {
      if (!mounted) return;
      _lessonOpen = false;
      ref.invalidate(worldProgressProvider);
  ...
  Widget _garden(BoxConstraints size, LessonProgress? p) => Padding(
      ...
      child: LetterJourney(progress: p, onAlif: () => _lesson()));

  Widget _review(LessonProgress? p) => Center(
      child: SingleChildScrollView(
          padding: const EdgeInsets.all(20),
          child: Column(children: [
            const Text('اختر لعبتك… وتعلّم مع كل محاولة',
                style: TextStyle(
              ...
              for (final game in ReviewGame.values)
                Expanded(
                    child: Padding(
                        ...
                        child: _Tile(
                            title: gameNames[game.index],
                            subtitle: gameDescriptions[game.index],
                            icon: Icons.extension_rounded,
                            artwork: ToyIcon(gameToys[game.index], size: 72),
                            onTap: () async {
                              await Navigator.of(context).push(
                                  MaterialPageRoute<void>(
                                      builder: (_) =>
                                          ReviewGameScreen(game: game)));
  ...
  Widget _parentContent(LessonProgress? p) => SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 12),
          const Text('نسخة تجريبية 39 • الملفات محلية، بلا حساب أو صور شخصية',
