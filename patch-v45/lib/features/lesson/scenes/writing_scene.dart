...
class _WritingSceneState extends ConsumerState<WritingScene> {
  ...
  Future<void> _validateFreeWriting() async {
    ...
    if (result.isValid) {
      final message = isAlif
          ? 'أحسنت، لقد كتبت حرف الألف بطريقة صحيحة ممتازة'
          : 'أحسنت، كتبت الحرف ونقاطه بطريقة جميلة';
      setState(() {
            ...
            children: [
              if (!_guided) ...[
                ...
                LessonActionButton(
                  ...
                  icon: _hasFreeInk ? Icons.check_rounded : Icons.west_rounded,
                ...
                if (!_attemptDone) const Spacer(),
                LessonActionButton(
                  label: _showDemo ? 'تخطي الشرح' : 'تخطي',
                  icon: Icons.west_rounded,
                  onPressed: _showDemo ? _skipDemo : _skipWriting,
                ),
              ],
            ],
          ),
