...
void main() {
  ...
  test(
      'review randomizes letters and words separately without chronological order',
      () {
    final questions = [
    ...
    for (var seed = 0; seed < 30; seed++) {
      ...
      expect(result.take(3).every((q) => q['kind'] == 'letter'), true);
      expect(result.skip(3).every((q) => q['kind'] == 'word'), true);
      expect(result.take(3).map((q) => q['reviewLessonId']).toList(),
          isNot(['alif', 'baa', 'taa']));
      expect(result.toSet(), questions.toSet());
  ...
  testWidgets(
      'review starts question only after introduction and skip cancels pending example',
      (tester) async {
    final audio = PendingAudio();
    final channel = SceneChannel();
    var skipped = 0;
