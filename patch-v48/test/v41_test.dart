...
void main() {
  ...
  testWidgets('Taa review covers both letters then words, with repeated audio',
      (tester) async {
    ...
    for (var i = 0; i < questions.length; i++) {
      ...
      await tester.tap(find
          .text(i == questions.length - 1 ? 'نستمع النشيد' : 'السؤال التالي'));
      await tester.pump();
    }
    expect(kinds, ['letter', 'letter', 'word', 'word']);
    expect(completed, 1);
    await tester.pumpWidget(const SizedBox());
    await tester.pump(const Duration(seconds: 1));
    channel.dispose();
