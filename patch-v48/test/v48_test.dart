...
void main() {
  ...
  test('Khaa content and checkpoint cover all seven letters', () {
    ...
    expect(scene.data['successAudio'],
        'assets/audio/taa/assessment_success.mp3');
    expect(scene.data['wrongAudio'],
        'assets/audio/checkpoint_1/checkpoint_wrong.mp3');
  });
  ...
  test('checkpoint unlocks after Khaa and gates the following group', () {
    ...
    expect(lessonIsUnlocked(lessons, 1, const {
      'khaa': LessonProgress(lessonId: 'khaa', completed: true),
    }), isTrue);
    expect(lessonIsUnlocked(lessons, 2, const {
      'khaa': LessonProgress(lessonId: 'khaa', completed: true),
    ...
    for (final size in [const Size(568, 320), const Size(844, 390)]) {
      ...
      for (final target in const [
        ...
        await tester.pumpWidget(support.app(
            LessonScreen(lessonId: target.$1, initialScene: target.$2)));
