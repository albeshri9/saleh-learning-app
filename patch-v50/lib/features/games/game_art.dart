...
class AtlasArt extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (!world) return SalehSticker(index);
...
class GameButton extends StatelessWidget {
  ...
  final bool selected, answerControl;
  final Color color;
                  ...
                  decoration: BoxDecoration(
                      gradient: LinearGradient(
                          ...
                          colors: selected
                              ...
                              : [
                                  Color.lerp(color, Colors.white, .91)!,
                                  Color.lerp(color, Colors.white, .78)!
                                ]),
                      borderRadius: BorderRadius.circular(22),
                      border: Border.all(
...
class _GameTap extends StatefulWidget {
  const _GameTap(
      {required this.child,
      required this.onTap,
      required this.borderRadius,
      required this.answerControl});
  final Widget child;
  final VoidCallback? onTap;
  final BorderRadius borderRadius;
  final bool answerControl;
  @override
  State<_GameTap> createState() => _GameTapState();
}

class _GameTapState extends State<_GameTap> {
  bool down = false;
  @override
  Widget build(BuildContext context) {
    if (!widget.answerControl) {
      return FeedbackTap(
          onTap: widget.onTap,
    ...
    return InkWell(
        ...
        child: AnimatedScale(
            scale: down && !MediaQuery.disableAnimationsOf(context) ? .96 : 1,
