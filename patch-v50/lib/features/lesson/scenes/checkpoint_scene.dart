...
List<Map<String, dynamic>> checkpointTaskFlow(
  ...
  if (data['standardizedFlow'] != true) return source;
...
class _CheckpointSceneState extends ConsumerState<CheckpointScene> {
  ...
  Future<void> _correctTask() async {
    ...
    _setFeedback(true, message: 'ممتاز! أحسنتم');
    _announceFeedback(true, nextPrompt: _currentPromptAudio);
  }
  ...
  void _skipDevelopmentalWriting() {
    _audioEpoch++;
    unawaited(_audio.stop());
    _advanceWithoutPraise();
  }
  ...
  Future<void> _chooseRemediation(int index) async {
    ...
    if (_remediationPhase == 0) {
      ...
      _setFeedback(true, message: 'ممتاز! أحسنتم');
      _announceFeedback(true, nextPrompt: data['wordAudio'] as String?);
      return;
    }
    ...
    _setFeedback(true, message: 'ممتاز! أحسنتم');
    _announceFeedback(true, nextPrompt: next['letterAudio'] as String?);
  }
  ...
  Widget _pronunciation() => Column(children: [
        ...
        Expanded(
            child: Center(
                child: SizedBox(
                    width: 150, height: 130, child: LetterGlyph(_letter)))),
        FeedbackTap(
  ...
  Widget _free() {
    final template =
        LetterTraceTemplate.fromId(_task['traceTemplateId'] as String?);
    final canSkipForAge = canSkipCheckpointFreeWriting(widget.api.profile.age);
    return Column(children: [
            ...
            children: [
              ...
              if (canSkipForAge) ...[
                ...
                LessonActionButton(
                    label: 'تخطي الكتابة',
                    icon: Icons.west_rounded,
                    onPressed: _busy ? null : _skipDevelopmentalWriting),
              ],
            ],
          )),
      if (canSkipForAge)
        const Text('خيار مناسب لعمر ٣–٤ سنوات',
            style: TextStyle(
                fontSize: 11,
                height: 1,
...
class _FeedbackStrip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedContainer(
        ...
        child: Text(
            value == null
                ? ''
                : message ?? (value! ? 'ممتاز! أحسنتم' : 'حاول مرة أخرى'),
            style: TextStyle(
                fontSize: 15,
                fontWeight: FontWeight.w800,
                color:
                      ...
                      ? LayoutBuilder(builder: (context, bounds) {
                          ...
                          return Center(
                              child: SizedBox.square(
                                  dimension: side, child: LetterGlyph(label)));
                        })
