...
void main() {
  ...
  testWidgets('capture all 36 actual game screens for visual review',
    ...
    for (var gameIndex = 0; gameIndex < gameCatalog.length; gameIndex++) {
      final g = gameCatalog[gameIndex];
      await tester.pumpWidget(fixtures.host(LearningGameScreen(
          key: ValueKey(g.id), game: g, random: Random(gameIndex + 1))));
    ...
    await tester.pumpAndSettle();
    await tester.tap(find.byKey(const ValueKey('item-w14')));
    await tester.pumpAndSettle();
    await expectLater(
