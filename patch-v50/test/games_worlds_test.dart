...
import 'package:saleh_app/features/games/game_stickers.dart';
...
void main() {
  ...
  test('first phase exposes exactly five original games per world', () {
    expect(firstPhaseGameIds.length, 15);
    for (final world in GameWorld.values) {
      expect(
          gameCatalog.where((g) => g.world == world && g.isFirstPhase).length,
          5);
    }
    expect(game('build_word').title, 'قطار الكلمة');
    expect(game('maze').level, 3);
    expect(game('tangram').level, 3);
  });
  testWidgets('all transparent vector stickers render over a dark background',
      (tester) async {
    await tester.pumpWidget(host(Container(
        color: const Color(0xFF3F365A),
        child: GridView.count(
            crossAxisCount: 5,
            children: [for (var i = 0; i < 15; i++) SalehSticker(i)]))));
    await tester.pumpAndSettle();
    expect(find.byType(SalehSticker), findsNWidgets(15));
  ...
  testWidgets('world hub and representative boards visual review',
    ...
    for (final id in ['picture_word', 'build_word', 'feed_rabbit', 'robot']) {
      await tester.pumpWidget(host(LearningGameScreen(
          key: ValueKey(id), game: game(id), random: Random(id.length))));
      await tester.pumpAndSettle();
      await tester.tap(find.byKey(const ValueKey('start-game')));
      await tester.pumpAndSettle();
