...
NEW = [
    dict(id='dal', name='الدال', spoken='دَا', letter='دَ', word='دَجَاجَة',
         image='chicken_v54.png', template='dal_fatha_pdf_v1'),
    dict(id='dhal', name='الذال', spoken='ذَا', letter='ذَ', word='ذَيْل',
         image='tail_v54.png', template='dhal_fatha_pdf_v1'),
    dict(id='raa', name='الراء', spoken='رَا', letter='رَ', word='رَجُل',
         image='man_v54.png', template='raa_fatha_pdf_v1'),
    dict(id='zay', name='الزاي', spoken='زَا', letter='زَ', word='زَرَافَة',
         image='giraffe_v54.png', template='zay_fatha_pdf_v1'),
    dict(id='seen', name='السين', spoken='سَا', letter='سَ', word='سَاعَة',
...
for position, item in enumerate(NEW):
    ...
    lesson.update(id=item['id'], title=f"درس حرف {item['letter']}")
    scenes = {scene['id']: scene for scene in lesson['scenes']}
    old_events = scenes['welcome_1']['lines'][0].get('events')
    welcome = f"مرحبًا يا أصدقائي، كيف حالكم اليوم؟ سوف نتعلم اليوم حرفًا جديدًا، حرف {item['spoken']}. هيا بنا."
    scenes['welcome_1']['lines'] = [line(item, 'welcome', welcome, old_events)]
    nasheed = f"الآن نستمع نشيد حرف {item['spoken']}."
    scenes['nasheed_1']['lines'] = [line(item, 'nasheed_intro', nasheed)]
    scenes['nasheed_1']['data']['label'] = f"أنشودة حرف {item['letter']}"
    explain = f"انظروا يا أصدقائي إلى حرف {item['spoken']}. {item['spoken']}، {item['word']}. {item['spoken']}، {item['word']}. رددوا معي: {item['spoken']}، {item['word']}."
    ...
    demo = f"انظروا كيف نكتب حرف {item['spoken']}. {shape}"
    attempt = f"هيا يا أصدقائي، اكتبوا حرف {item['spoken']}. ضعوا إصبعكم على النقطة الخضراء، واتبعوا المسار حتى النهاية."
    ...
    free = f"هيا يا أصدقائي، اكتبوا حرف {item['spoken']} كتابة حرة الآن."
    scenes['write_free_1']['lines'] = [line(item, 'free_intro', free)]
    scenes['write_free_1']['canSkip'] = False
    scenes['write_free_1']['data'].update(
        letter=item['letter'], lessonId=item['id'], traceTemplateId=item['template'])
    letter_q = f"أين حرف {item['spoken']}؟ اضغطوا على حرف {item['spoken']}."
    word_q = f"أين الصورة التي تبدأ بحرف {item['spoken']}؟ اختاروا الصورة الصحيحة."
    ...
    closing = f"أبدعتم يا أبطال. لقد تعلمنا اليوم حرف {item['spoken']}. {item['spoken']}، {item['word']}. إلى اللقاء يا أصدقائي."
