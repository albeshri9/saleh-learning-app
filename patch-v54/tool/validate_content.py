...
def validate(root=ROOT):
    ...
    for pack in catalog['packs']:
        ...
        if stage_id:
            ...
            expected_prior = ([lesson_id for lesson_id in ordered[:ordered.index(pid)]
                               if not lesson_id.startswith('checkpoint_')]
                              if pid in ordered else [])
            require(pack.get('priorLessonIds') == expected_prior, f'{pid}: incomplete cumulative review')
            if pack.get('kind') != 'checkpoint':
