...
/// Generous template coverage for early writers. Dot regions must each contain
/// a separate short stroke, so a horizontal scribble cannot substitute for dots.
WritingValidationResult validateDottedLetterWriting(
  ...
  if (size.isEmpty || !size.width.isFinite || !size.height.isFinite) {
    return const WritingValidationResult(
        isValid: false, score: 0, reason: 'missingParts');
  }
  final rect = template.drawingRect(size);
  final ink = <Offset>[];
  for (final stroke in sample.strokes) {
    if (stroke.isEmpty) continue;
    ink.add(stroke.first);
    for (var i = 1; i < stroke.length; i++) {
      final a = stroke[i - 1], b = stroke[i];
      final steps = ((b - a).distance / 3).ceil().clamp(1, 500);
      for (var j = 1; j <= steps; j++) {
        ink.add(Offset.lerp(a, b, j / steps)!);
      }
    }
  }
  if (ink.isEmpty) {
  ...
  var covered = 0.0;
  final usedDots = <int>{};
  final expectedDotParts = template.guideParts.where((part) => part.isDot);
  final dotCenters = expectedDotParts
      .map((part) => part.samples(rect, count: 1).first)
      .toList(growable: false);
  for (final part in template.guideParts) {
    final targets = part.samples(rect, count: 36);
    final tolerance = rect.width * (part.isDot ? .10 : .095);
    double coverage;
    if (part.isDot) {
      var hit = false;
      for (var i = 0; i < sample.strokes.length; i++) {
        final stroke = sample.strokes[i];
        if (stroke.isEmpty || usedDots.contains(i)) continue;
        var length = 0.0;
        for (var j = 1; j < stroke.length; j++) {
          length += (stroke[j] - stroke[j - 1]).distance;
        }
        if (length < rect.width * .34 &&
            stroke.any((p) => (p - targets.first).distance < tolerance)) {
          hit = true;
          usedDots.add(i);
          break;
        }
      ...
      coverage = targets
              .where((p) => ink.any((q) => (p - q).distance <= tolerance))
              .length /
          targets.length;
    }
    covered += coverage;
    if (coverage < (part.id == 'body' ? .62 : .45)) missing.add(part.id);
  }

  // The old check stopped after finding the required dots. That meant a baa
  // with two dots below could still pass because one of the two was enough to
  // satisfy dot1. Count every separate compact mark in the expected dot zone
  // and require an exact one-to-one match. The generous radius intentionally
  // accepts imprecise children's dots while still distinguishing ب from ي,
  // ت from ث, and ح from ج/خ. The fatha remains a separate required guide part
  // above, so omitting it cannot be hidden by a dot or by the body stroke.
  if (dotCenters.isNotEmpty) {
    final dotRadius = rect.width * .145;
    var drawnDots = 0;
    for (final stroke in sample.strokes) {
      if (stroke.isEmpty) continue;
      var length = 0.0;
      ...
      final compact = length < rect.width * .34;
      final inDotZone = stroke.any(
        (point) => dotCenters.any(
          (center) => (point - center).distance <= dotRadius,
        ),
      );
      if (compact && inDotZone) drawnDots++;
    }
    if (drawnDots != dotCenters.length) {
      missing.add(drawnDots < dotCenters.length ? 'dots' : 'extraDots');
    }
  }

  final uniqueMissing = missing.toSet().toList(growable: false);
  return WritingValidationResult(
      isValid: uniqueMissing.isEmpty,
      score: covered / template.parts.length,
      missingParts: uniqueMissing,
      reason: uniqueMissing.isEmpty ? null : 'missingParts');
}

/// فحص متسامح للكتابة الحرة داخل الاختبار المرحلي.
///
/// لا نطلب من الطفل الكتابة فوق نموذج الحرف أو في موضع محدد من اللوحة.
/// نطبّع رسمه بحسب حدوده الخاصة أولًا، ثم نقارنه ببنية الحرف بنسبة إتقان
/// قدرها 50٪. تبقى الأجزاء الأساسية إلزامية: جسم الحرف، والفتحة، والعدد
/// الصحيح من النقاط؛ لذلك لا تتحول السماحية إلى قبول حرف ناقص.
WritingValidationResult validateCheckpointLetterWriting(
  ...
  final strokes = sample.strokes
      .map((stroke) => _normalizeAndClean(stroke, size))
      .where((stroke) =>
          stroke.isNotEmpty &&
          (stroke.length == 1 || _pathLength(stroke) >= .004))
      .toList(growable: false);
  if (strokes.isEmpty) {
    return const WritingValidationResult(
      isValid: false,
  ...
  final inkBounds = _boundsOf(strokes.expand((stroke) => stroke).toList());
  if (math.max(inkBounds.width, inkBounds.height) < .10) {
    return const WritingValidationResult(
      isValid: false,
      score: .1,
      reason: 'tooSmall',
  ...
  Offset relative(Offset point, Rect bounds) => Offset(
        (point.dx - bounds.left) / math.max(.001, bounds.width),
        (point.dy - bounds.top) / math.max(.001, bounds.height),
      );

  final relativeStrokes = strokes
      .map((stroke) =>
          stroke.map((point) => relative(point, inkBounds)).toList())
      .toList(growable: false);
  final denseRelativeStrokes = <List<Offset>>[];
  for (final stroke in relativeStrokes) {
    final denseStroke = <Offset>[stroke.first];
    for (var index = 1; index < stroke.length; index++) {
      final a = stroke[index - 1];
      final b = stroke[index];
      final steps = ((b - a).distance / .012).ceil().clamp(1, 160);
      for (var step = 1; step <= steps; step++) {
        denseStroke.add(Offset.lerp(a, b, step / steps)!);
      }
    }
    denseRelativeStrokes.add(denseStroke);
  }
  const unitRect = Rect.fromLTWH(0, 0, 1, 1);
  final rawTargets = <String, List<Offset>>{
    for (final part in template.guideParts)
      part.id: part.samples(unitRect, count: 64),
  };
  final targetBounds =
      _boundsOf(rawTargets.values.expand((points) => points).toList());
  final targets = <String, List<Offset>>{
    for (final entry in rawTargets.entries)
      entry.key: entry.value
          .map((point) => relative(point, targetBounds))
          .toList(growable: false),
  };
  ...
  double coverageFor(
    ...
    double tolerance = .16,
      ...
      expected
          .where((target) =>
              ink.any((point) => (point - target).distance <= tolerance))
          .length /
      math.max(1, expected.length);

  var bodyStrokeIndex = 0;
  var bestBodyCoverage = -1.0;
  final bodyTarget = targets['body'];
  for (var index = 0; index < denseRelativeStrokes.length; index++) {
    final coverage = bodyTarget == null
        ? _pathLength(relativeStrokes[index])
        : coverageFor(bodyTarget, denseRelativeStrokes[index]);
    if (coverage > bestBodyCoverage) {
      bestBodyCoverage = coverage;
      bodyStrokeIndex = index;
    }
  }

  final missing = <String>[];
  var scoreTotal = 0.0;
  final assignedStrokeIndexes = <int>{bodyStrokeIndex};
  for (final part in template.guideParts) {
    ...
    if (part.id == 'body') {
      coverage = coverageFor(
        targets[part.id]!,
        denseRelativeStrokes[bodyStrokeIndex],
      );
    } else {
      var selectedIndex = -1;
      for (var index = 0; index < denseRelativeStrokes.length; index++) {
        if (assignedStrokeIndexes.contains(index)) continue;
        // A tap is a dot, not a substitute for the fatha or hamza stroke.
        if (_pathLength(relativeStrokes[index]) < .055) continue;
        final candidate =
            coverageFor(targets[part.id]!, denseRelativeStrokes[index]);
        if (candidate > coverage) {
          coverage = candidate;
          selectedIndex = index;
        }
      }
      if (selectedIndex >= 0) assignedStrokeIndexes.add(selectedIndex);
    }
    scoreTotal += coverage;
    final minimum = part.id == 'body' ? .45 : .30;
    if (coverage < minimum) missing.add(part.id);
  }

  final expectedDots = template.guideParts.where((part) => part.isDot).toList();
  if (expectedDots.isNotEmpty) {
    final dotCenters = expectedDots
        .map((part) => targets[part.id]!.first)
        .toList(growable: false);
    final fathaCenter = _averageOffset(targets['fatha'] ?? const []);
    var drawnDots = 0;
    for (final stroke in relativeStrokes) {
      final length = _pathLength(stroke);
      if (length > .42) continue;
      final center = _averageOffset(stroke);
      final dotDistance = dotCenters
          .map((target) => (center - target).distance)
          .fold<double>(double.infinity, math.min);
      final fathaDistance = (center - fathaCenter).distance;
      if (dotDistance <= .22 && dotDistance < fathaDistance) drawnDots++;
    }
    final exactDots = drawnDots == expectedDots.length;
    scoreTotal += exactDots ? expectedDots.length : 0;
    if (!exactDots) {
      missing.add(drawnDots < expectedDots.length ? 'dots' : 'extraDots');
    }
  }

  final partCount = template.guideParts.length;
  final score = (scoreTotal / math.max(1, partCount)).clamp(0.0, 1.0);
  final uniqueMissing = missing.toSet().toList(growable: false);
  final valid = uniqueMissing.isEmpty && score >= .50;
  return WritingValidationResult(
    isValid: valid,
    score: score,
    missingParts: uniqueMissing,
    reason: valid ? null : 'lowConfidence',
  );
}
...
/// جسم ألف طويل وواضح، وضربتان قصيرتان أعلى الجسم تدلان على الهمزة والفتحة.
/// لا تكفي إحداهما عن الأخرى، لكننا لا نفرض دقة هندسية عالية على رسم الطفل.
/// نبقي فقط
...
WritingValidationResult validateAlifFathaChildFriendly(WritingSample sample) {
  ...
  if (upperInk.length != 2) {
    return const WritingValidationResult(
      isValid: false,
      score: .46,
      missingParts: ['hamza', 'fatha'],
      reason: 'missingParts',
    );
  }
  ...
  final notScribble = normalized.length <= 5 && totalLength <= 5.2;
