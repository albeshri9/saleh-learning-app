...
class _GameBoardState extends State<GameBoard> {
  ...
  Widget _grid(List<Widget> children, {int columns = 3}) =>
      LayoutBuilder(builder: (context, c) {
        ...
        return GridView.count(
            ...
            mainAxisSpacing: 6,
            crossAxisSpacing: 6,
            childAspectRatio: ((c.maxWidth - 8 - (columns - 1) * 6) / columns) /
                max(44, (c.maxHeight - 12 - (rows - 1) * 6) / max(1, rows)),
  ...
  Widget _feeding() => Column(children: [
        Expanded(
          ...
          Expanded(
              flex: 3,
              child: Column(children: [
                const Text('اسحب الجزرة أو اضغطها',
                    textAlign: TextAlign.center,
                    style:
                        TextStyle(color: gameInk, fontSize: 13, height: 1.15)),
                Expanded(
                    child: _grid([
                  for (var i = 0; i < 6; i++)
                    Draggable<int>(
  ...
  Widget _counter() => Row(children: [
        ...
        Expanded(
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Expanded(
                child: _action('أضف', () => s.adjustCount(1),
                    icon: Icons.add_rounded)),
            const SizedBox(width: 4),
            Expanded(
                child: _action('أعد', () => s.adjustCount(-1),
  ...
  Widget _mirror() => Row(children: [
        ...
        const SizedBox(width: 8),
        SizedBox(width: 90, child: _action('تحقق من التناظر', s.checkMirror))
      ]);
