...
const gameCatalog = <GameSpec>[
  ...
  GameSpec('picture_market', 'سوق الصور', 'أختار اسم المنتج', GameWorld.words,
      PlayKind.choose, 5),
  GameSpec('sentence', 'ابنِ جَمَلة', 'أرتب كلمات جَمَلة', GameWorld.words,
      PlayKind.order, 1),
  GameSpec('feed_rabbit', 'أطعم الأَرنب', 'أعد قطع الجزر', GameWorld.numbers,
      PlayKind.counter, 1),
...
GameRound roundFor(GameSpec game, int round) {
  ...
  final wordLetters = w.characters.toList();
  switch (game.id) {
    case 'picture_word':
      return GameRound(
    ...
    case 'letter_basket':
      return GameRound(
          prompt: 'اجمع الصور التي تبدأ بـ «${['أَ', 'بَ', 'تَ'][r]}»',
      ...
      return GameRound(
          ...
          items: [
            for (var i = 0; i < wordLetters.length; i++)
              GameItem('c$i', text: wordLetters[i])
          ],
          answer: [for (var i = 0; i < wordLetters.length; i++) 'c$i'],
          hint: 'ابدأ من اليمين');
    case 'missing_letter':
      return GameRound(
          prompt: 'ما الحرف الناقص؟',
          hero: animal,
          caption: ['أَ ــ د', 'بَ ــ ة', 'ثَ ــ ل ب'][r],
          items: [
    ...
    case 'word_train':
      return GameRound(
          ...
          items: [
            ...
            GameItem('letter', text: wordLetters.first)
          ],
    ...
    case 'feed_rabbit':
      return GameRound(
          prompt: 'أعط الأَرنب ${digits(r + 3)} جزرات',
          hero: 1,
          target: r + 3,
          a: 6,
    ...
    case 'positions':
      return GameRound(
          prompt: 'الأَرنب في الوسط والأَسد على يمينه',
          items: [
            word(0, image: true),
            word(1, image: true),
            word(2, image: true)
          ...
          hint: 'المواضع من اليمين: أَسد، أَرنب، بَطة');
    default:
