                                  ...
                                  Expanded(
                                    ...
                                    Positioned.fill(
                                        child: LetterGlyph(
                                            ['أَ', 'بَ', 'تَ', 'ثَ'][i],
                                            color: profileInk)),
                                    if (i == 0 && progress?.completed == true)
                                      const PositionedDirectional(
                                          top: 0,
                                          end: 0,
                                          child: CompletionBadge(
                                              key: ValueKey(
                                                  'letter-completed-alif'))),
                                  ])),
                                  Text(
                                      i == 0
          ...
          data: (journal) {
            final badges = [
              (
                'مستكشف الحروف',
                Toy.book,
                journal.awards.keys.any((k) => k.endsWith(':explain_1'))
              ),
              (
                'قلمي الجميل',
                Toy.pencil,
                journal.awards.keys.any((k) => k.endsWith(':write_guided_1'))
              ),
              (
                'بطل الألعاب',
                Toy.puzzle,
              ...
              (
                'أكملت الدرس',
                Toy.trophy,
                journal.awards.keys.any(
                    (k) => k.startsWith('lesson:') && k.endsWith(':complete'))
              ),
            ];
            return LayoutBuilder(builder: (context, box) {
              final compact = box.maxHeight < 340;
              return Padding(
                ...
                child: Column(children: [
                  SizedBox(
                      ...
                      child: Row(children: [
                        Expanded(
                            child: _stat(Toy.coins, '${journal.points}',
                                'رصيد النقاط', compact)),
                  ...
                  const Text('10 نقاط للنشاط • 15 للعبة • 20 لإكمال الدرس',
                      maxLines: 1,
                      textAlign: TextAlign.center,
                      style: TextStyle(
                          fontSize: 12, height: 1.15, color: profileInk)),
                ]),
              );
            });
          });
  Widget _stat(Toy toy, String value, String label, bool compact) => Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
      decoration: BoxDecoration(
          color: const Color(0xFAF8F6FD),
          borderRadius: BorderRadius.circular(18)),
      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        ToyIcon(toy, size: compact ? 30 : 46),
        ...
        Flexible(
          ...
          Text(label,
              ...
              style: TextStyle(
                  fontSize: compact ? 12 : 14, height: 1.2, color: profileInk)),
        ])),
      ]));
}
