...
class LearningJournal {
  ...
  Future<void> award(String id, int points) =>
      _update((old) => old.awards.containsKey(id)
          ? old
          : JournalData(
              awards: {...old.awards, id: points}, drawings: old.drawings));
...
final journalDataProvider = FutureProvider.autoDispose((ref) async {
  ...
  final records = await ref.watch(allLessonProgressProvider.future);
  final current = await journal.load();
  final earned = <String, int>{
    for (final entry in records.entries) ...{
      for (final id in entry.value?.completedScenes ?? <String>[])
        if (!id.startsWith('welcome') &&
            !id.startsWith('nasheed') &&
            !id.startsWith('success'))
          'lesson:${entry.key}:$id': 10,
      if (entry.value?.completed == true) 'lesson:${entry.key}:complete': 20,
    },
  };
