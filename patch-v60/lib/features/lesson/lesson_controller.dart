...
class LessonController
  ...
  late LearningJournal _journal;
  LessonProgress? _saved;
  final Set<String> _completedScenes = {};
  // Reading scores reset on replay. Historical scene completion must not make
  // a skipped replay pass with just one correct answer in the current run.
  final Set<String> _completedThisRun = {};
  @override
  Future<LessonRunState> build(String arg) async {
    _disposed = false;
    ref.onDispose(() => _disposed = true);
    _repository = ref.watch(progressRepositoryProvider);
    _journal = ref.read(journalProvider);
    ...
    // Historical points are reconciled by the journal view. Opening a lesson
    // must never rewrite the child's entire drawing portfolio for each award.
    final startIndex = (saved != null &&
            !saved.completed &&
            saved.lastSceneIndex >= 0 &&
  ...
  Future<void> completeScene({bool skipped = false}) async {
    ...
    _transitioning = true;
    if (!skipped) {
      _completedScenes.add(s.currentScene.id);
      _completedThisRun.add(s.currentScene.id);
    }
    final nextIndex = s.sceneIndex + 1;
    try {
      if (!skipped &&
          ![SceneType.welcome, SceneType.nasheed, SceneType.success]
              .contains(s.currentScene.type)) {
        _awardInBackground('lesson:${s.lesson.id}:${s.currentScene.id}', 10);
      }
      if (nextIndex >= s.lesson.scenes.length) {
        await _finish(s);
      } else {
        final next = s.copyWith(sceneIndex: nextIndex);
        // Only the small progress record is on the critical path, never the
        // portfolio. The completion badge is persisted before success appears.
        await _persist(next);
  ...
  /// Previous-letter review is practice, not part of this letter's score.
  void recordAnswer({required bool correct}) {
    final s = _s;
    if (s == null || s.currentScene.type == SceneType.review) return;
  ...
  Future<void> _finish(LessonRunState s) async {
    final readingComplete = !s.lesson.id.startsWith('reading_') ||
        (s.totalQuestions > 0 &&
            s.lesson.mastery.requiredSceneIds
                .every(_completedThisRun.contains));
    final mastered = readingComplete && s.score >= s.lesson.mastery.minScore;
    await _repository
        .saveLessonProgress(
          LessonProgress(
    ...
    if (!_disposed) state = AsyncData(s.copyWith(finished: true));
    _awardInBackground('lesson:${s.lesson.id}:complete', 20);
  }

  void _awardInBackground(String id, int points) {
    unawaited(_journal.award(id, points).then((_) {
      if (!_disposed) ref.invalidate(journalDataProvider);
    }).catchError((Object error) {
      // Completed activities are already persisted and can restore their points
      // later. A secondary reward failure must not block the lesson or exit.
      debugPrint('Reward save deferred: $error');
    }));
  }
        ...
        .saveLessonProgress(
          LessonProgress(
            ...
            completed: (_saved?.completed ?? false) ||
                s.currentScene.type == SceneType.success,
            mastered: s.currentScene.type == SceneType.success
                ? s.score >= s.lesson.mastery.minScore
                : _saved?.mastered ?? false,
            score: s.score,
            attempts: s.attempts,
            completedScenes: _completedScenes.toList(),
          ),
