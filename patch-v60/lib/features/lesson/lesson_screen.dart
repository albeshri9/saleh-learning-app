...
class _LessonScreenState extends ConsumerState<LessonScreen> {
  ...
  bool _closing = false;
  bool _readingExitScheduled = false;
  ...
  Widget build(BuildContext context) {
    ref.listen(lessonControllerProvider(widget.lessonId), (_, next) {
      if (widget.lessonId.startsWith('reading_') &&
          next.valueOrNull?.finished == true &&
          !_readingExitScheduled) {
        _readingExitScheduled = true;
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) _exitLesson(context);
        });
      }
    });
                ...
                builder: (context, constraints) {
                  if (constraints.maxWidth < 1100 ||
                    ...
                    return _CompactLesson(
                      ...
                      onSkip: lessonSceneCanSkip(scene) ? api.skipScene : null,
            ...
            child: Column(children: [
              SizedBox(
                  ...
                  child: _CompactTopBar(
                      state: state, api: api, onExit: onExit, onSkip: onSkip)),
                  ...
                  Expanded(
                    child: SizedBox(
                      ...
                      child: _LessonBoard(
                        ...
                        child: Padding(
                          padding: EdgeInsets.only(
                              bottom: scene.type == SceneType.reading ? 10 : 0),
                          child: SceneRegistry.build(scene, api),
                        ),
                      ),
                    ),
...
class _CompactTopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      Row(textDirection: TextDirection.rtl, children: [
        ...
        if (onSkip != null)
          TextButton.icon(
            key: const ValueKey('lesson-global-skip'),
            onPressed: onSkip,
            icon: const Icon(Icons.west_rounded, size: 18),
            label: const Text('تخطي'),
            style: TextButton.styleFrom(
              foregroundColor: const Color(0xFF6F55A0),
...
@visibleForTesting
bool lessonSceneCanSkip(Scene scene) =>
    ExperimentalRelease.skipEveryLessonSection || scene.canSkip;
