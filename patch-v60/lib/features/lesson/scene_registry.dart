...
import 'scenes/checkpoint_scene.dart';
import 'scenes/nasheed_scene.dart';
import 'scenes/pronunciation_scene.dart';
import 'scenes/reading_assessment_scene.dart';
...
class SceneChannel {
  ...
  final ValueNotifier<bool> scriptInterrupted = ValueNotifier(false);

  void interruptScript() => scriptInterrupted.value = true;
...
class SceneApi {
  ...
  final FutureOr<void> Function() completeScene;
  final FutureOr<void> Function()? skipScene;
  FutureOr<void> skip() => (skipScene ?? completeScene)();
...
abstract class SceneRegistry {
  static final Map<SceneType, SceneWidgetBuilder> _builders = {
    ...
    SceneType.reading: (s, a) => ReadingAssessmentScene(scene: s, api: a),
    ...
    SceneType.checkpoint: (s, a) => CheckpointScene(scene: s, api: a),
