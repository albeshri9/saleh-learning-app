...
class _McqSceneState extends ConsumerState<McqScene> {
  late final List<Map<String, dynamic>> _sourceQuestions =
      ((widget.scene.data['questions'] as List?) ?? const [])
          .cast<Map<String, dynamic>>();
  late final List<Map<String, dynamic>> _questions = _isReview
      ? reviewQuestionOrder(_sourceQuestions, _random)
      : _sourceQuestions;
  ...
  bool get _waitsForIntro =>
      _isReview || widget.scene.data['questionAfterIntro'] == true;
  int _audioEpoch = 0;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (mounted && _waitsForIntro) _introFinished();
    });
  }
  ...
  void _introFinished() {
    ...
    setState(() => _reviewStarted = true);
    if (_questionAudio != null) {
      unawaited(ref.read(audioServiceProvider).play(_questionAudio!));
    }
  }
  ...
  Future<void> _answerExample(
      Future<void> feedback, int epoch, String? example) async {
    await feedback;
    if (mounted && epoch == _audioEpoch && example != null) {
      await ref.read(audioServiceProvider).play(example);
    }
  }
  ...
  bool get _isReview => widget.scene.type == SceneType.review;
  bool get _showPrompt => !_isReview && (_q['showPrompt'] as bool? ?? true);

  String? get _questionAudio => (_q['audio'] ??
      (_questionIndex == 1 ? widget.scene.data['secondAudio'] : null) ??
      (_questionIndex == 0 && widget.scene.lines.isNotEmpty
          ? widget.scene.lines.last.audio
          : null)) as String?;

  Future<void> _choose(int index) async {
    if (_answered || (_waitsForIntro && !_reviewStarted)) return;
    _reviewStarted = true;
    ...
    if (correct) {
      ...
      unawaited(_answerExample(
          feedbackPlayback, epoch, _q['answerAudio'] as String?));
      return;
    }
    await Future.wait([
      feedbackPlayback,
    ...
    if (!mounted || correct || epoch != _audioEpoch) return;
    ...
    if (_isReview && _questionAudio != null) {
      // Feedback has finished. The same question repeats, with no written
      // answer clue and without waiting before accepting another choice.
      unawaited(ref.read(audioServiceProvider).play(_questionAudio!));
    }
  }
                                ...
                                state: !_answered
                                    ...
                                    : correct && i == _correctIndex
                                        ...
                                        : i == _selected
                                            ? _OptionState.wrong
                                            : _OptionState.dimmed,
                                onTap: _waitsForIntro && !_reviewStarted
                                    ? null
                                    : () => _choose(i),
                              ),
                            ),
