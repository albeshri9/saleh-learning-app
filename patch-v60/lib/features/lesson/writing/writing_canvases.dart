...
class _GuidedTracingCanvasState extends State<GuidedTracingCanvas> {
  ...
  void _advanceStroke() {
    if (InteractionEffects.animate(context)) {
      _glowingStroke = _strokeIndex;
      _glowTimer?.cancel();
      _glowTimer = Timer(const Duration(milliseconds: 350), () {
        if (mounted) setState(() => _glowingStroke = null);
      });
    }
        ...
        return GestureDetector(
          ...
          child: TweenAnimationBuilder<double>(
            ...
            builder: (context, displayedProgress, child) {
              ...
              return CustomPaint(
                foregroundPainter:
                    _StrokeGlow(widget.traceTemplate, _glowingStroke),
                painter: _StrokesPainter(
                  letter: widget.letter,
                  strokes: widget.strokes,
                  traceTemplate: widget.traceTemplate,
...
class _StrokeGlow extends CustomPainter {
  _StrokeGlow(this.template, this.index);
  final LetterTraceTemplate? template;
  final int? index;
  @override
  void paint(Canvas canvas, Size size) {
    if (template == null || index == null) return;
    final part = template!.guideParts[index!];
    canvas.drawPath(
        part.outlinePath(template!.drawingRect(size)),
        Paint()
          ..color = const Color(0x8885E9AF)
          ..maskFilter = const MaskFilter.blur(BlurStyle.outer, 5));
  }

  @override
  bool shouldRepaint(_StrokeGlow old) =>
      old.index != index || old.template != template;
}

/// الكتابة الحرة: حبر حر بلا دليل، مع مسح وإنهاء.
class FreeWritingCanvas extends StatefulWidget {
  const FreeWritingCanvas({super.key, this.onInkChanged, this.traceTemplate});
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return GestureDetector(
          ...
          child: CustomPaint(
            foregroundPainter: _InkPainter(lines: _lines),
            painter: widget.traceTemplate == null
                ? null
                : FreeLetterGuide(widget.traceTemplate!),
            child: const SizedBox.expand(),
          ),
        );
      },
...
void paintTracePartReveal(
  ...
  if (part.isDot && progress > 0) {
    canvas.drawPath(
        outline, Paint()..color = color.withValues(alpha: progress));
    return;
  }
  if (progress >= .999) {
  ...
  final paint = Paint()
    ..color = color
    ..style = PaintingStyle.stroke
    ..strokeWidth = part.revealStrokeWidth(drawingRect)
    ..strokeCap = StrokeCap.round
    ..strokeJoin = StrokeJoin.round;
...
class CompletedTracingCanvas extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      foregroundPainter: glow && traceTemplate != null
          ? CompletedLetterGlow(traceTemplate!)
          : null,
      painter: _StrokesPainter(
        letter: letter,
        strokes: strokes,
...
/// Same approved silhouette as the tracing guide; never snaps the child's ink.
class FreeLetterGuide extends CustomPainter {
  const FreeLetterGuide(this.template);
  final LetterTraceTemplate template;
  @override
  void paint(Canvas canvas, Size size) {
    final rect = template.drawingRect(size);
    for (final part in template.parts) {
      canvas.drawPath(
          part.outlinePath(rect), Paint()..color = const Color(0x147960AD));
    }
    _paintTemplateOutline(canvas, size, template);
  }

  @override
  bool shouldRepaint(FreeLetterGuide old) => old.template != template;
}

/// A visible golden halo follows the completed letter, not the panel border.
class CompletedLetterGlow extends CustomPainter {
  const CompletedLetterGlow(this.template);
  final LetterTraceTemplate template;
  @override
  void paint(Canvas canvas, Size size) {
    for (final part in template.parts) {
    ...
    for (final delta in [
      ...
      final p = Offset(rect.center.dx + rect.width * delta.dx,
          rect.top + rect.height * delta.dy);
  ...
  @override
  bool shouldRepaint(CompletedLetterGlow old) => old.template != template;
}
