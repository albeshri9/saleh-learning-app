...
void main() {
  ...
  test('الفحص الطفولي يتطلب جسم الألف والهمزة والفتحة', () {
    const strokes = [
      [
        Offset(.50, .28),
        Offset(.49, .44),
    ...
    final result = validateAlifFathaChildFriendly(sample(strokes));
    final missingFatha =
        validateAlifFathaChildFriendly(sample(strokes.sublist(0, 2)));

    expect(result.isValid, isTrue, reason: result.reason);
    expect(missingFatha.isValid, isFalse);
  });
