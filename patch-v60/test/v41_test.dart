...
void main() {
  ...
  testWidgets('early review answer cancels queued narration and startup retry',
    ...
    expect(audio.assets, ['assets/audio/v45/review_intro.mp3']);
    stopped.value = true;
    await audio.play('assets/audio/taa/assessment_success.mp3');
    await tester.pump(const Duration(seconds: 3));
    expect(audio.assets, [
      'assets/audio/v45/review_intro.mp3',
  ...
  test('seven ordered stages and current first-group lessons are ordered',
      () async {
    ...
    final fathaLevels = programs.single.stages.first.levels;
    expect(fathaLevels.first.lessons.map((l) => l.lessonId),
        [
          'alif',
          'baa',
          'taa',
    ...
    expect(fathaLevels[1].lessons.map((l) => l.lessonId), [
      'dal',
      'dhal',
      'raa',
      'zay',
    ...
    for (final id in ['baa', 'taa']) {
      ...
      expect(
          (review.data['questions'] as List)
              .where((q) => q['kind'] == 'letter')
              .map((q) => q['reviewLessonId']),
          expected);
  ...
  testWidgets('Taa review covers each prior letter once across glyph and word',
      (tester) async {
    ...
    final expectedCount = (scene.data['priorLessonIds'] as List).length;
    final kinds = <String>[];
    final reviewedLessons = <String>[];
    for (var i = 0; i < expectedCount; i++) {
      final current =
          questions.singleWhere((q) => q['audio'] == audio.assets.last);
      kinds.add(current['kind'] as String);
      reviewedLessons.add(current['reviewLessonId'] as String);
      if (i == 0) {
        final repeated = audio.assets.last;
        await tester.tap(glyph('تَ'));
        await tester.pump(const Duration(seconds: 1));
        await tester.pump();
        expect(audio.assets.sublist(audio.assets.length - 2),
            ['assets/audio/taa/assessment_retry.mp3', repeated]);
      }
      final answer = (current['options']
          as List)[current['correctIndex'] as int] as String;
      await tester
          .tap(current['kind'] == 'word' ? find.text(answer) : glyph(answer));
      await tester.pump();
      await tester.tap(
          find.text(i == expectedCount - 1 ? 'نستمع النشيد' : 'السؤال التالي'));
      await tester.pump();
    }
    expect(kinds, ['letter', 'word']);
    expect(reviewedLessons.toSet(), {'alif', 'baa'});
    expect(completed, 1);
    await tester.pumpWidget(const SizedBox());
    await tester.pump(const Duration(seconds: 1));
    channel.dispose();
