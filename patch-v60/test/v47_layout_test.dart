...
void main() {
  ...
  final screens = <String, Widget>{
    ...
    for (final world in GameWorld.values)
      'world-${world.name}': GamesWorldScreen(world: world),
  };
  for (final size in [
    const Size(568, 320),
    const Size(800, 360),
  ...
  for (final size in [const Size(568, 320), const Size(800, 360)]) {
    ...
    testWidgets('all 36 games fit $size including the active board',
        (tester) async {
      final hiddenRows = <String>[];
      support.viewport(tester, size);
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
      for (final game in gameCatalog) {
        await tester
            .pumpWidget(auditApp(LearningGameScreen(game: game), scale: 2));
        await support.frames(tester, 6);
        expect(tester.takeException(), isNull, reason: '${game.id} intro');
        await tester.tap(find.byKey(const ValueKey('start-game')));
        await support.frames(tester, 6);
        expect(tester.takeException(), isNull, reason: '${game.id} board');
        for (final state
            in tester.stateList<ScrollableState>(find.byType(Scrollable))) {
          if (axisDirectionToAxis(state.position.axisDirection) ==
                  Axis.vertical &&
              state.position.maxScrollExtent > .5) {
            hiddenRows.add(
                '${game.id}: ${state.position.maxScrollExtent} hidden pixels');
          }
        }
        await tester.pumpWidget(const SizedBox());
      }
  ...
  testWidgets('v47 all game boards visual audit', (tester) async {
    ...
    addTearDown(tester.view.resetDevicePixelRatio);
    for (final game in gameCatalog) {
      await tester.pumpWidget(auditApp(LearningGameScreen(game: game)));
      await support.frames(tester, 6);
      await tester.tap(find.byKey(const ValueKey('start-game')));
      await support.frames(tester, 6);
