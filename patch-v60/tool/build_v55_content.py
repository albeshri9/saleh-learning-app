...
def narration(item):
    ...
    return {
        ...
        "assessment_letter": f"أين حرف {assessment}؟ اضغطوا على حرف {assessment}.",
        "assessment_word": f"أين الصورة التي تبدأ بحرف {assessment}؟ اختاروا الصورة الصحيحة.",
    ...
    for index, target in enumerate(prior):
        ...
        if use_word:
            question = {
                ...
                "showPrompt": False,
                "prompt": f"أين الصورة التي تبدأ بحرف {target['assessment']}؟",
            ...
            question = {
                ...
                "showPrompt": False, "prompt": f"أين حرف {target['assessment']}؟",
...
def build_lesson(item, prior):
    ...
    lesson["id"] = item["id"]
    lesson["title"] = f"درس حرف {item['article']}"
...
def main():
    ...
    for index, item in enumerate(LETTERS):
        pack = by_id[item["id"]]
        pack.update(
            title=f"حرف {item['name']}", letter=item["letter"],
