...
class _CheckpointSceneState extends ConsumerState<CheckpointScene> {
  ...
  Future<void> _hearPronunciation() async {
    final asset = fathaTapRepeatAssetFor(_letter);
    if (asset == null || _pronunciationAttempts < 2) return;
    final request = ++_speechRequest;
    final epoch = ++_audioEpoch;
    widget.api.channel.interruptScript();
    InteractionAudio.stopCelebration();
    ...
    if (!mounted || request != _speechRequest || epoch != _audioEpoch) return;
  ...
  Widget _pronunciation() => Column(children: [
        ...
        Row(
            textDirection: TextDirection.rtl,
            mainAxisAlignment: _pronunciationAttempts >= 2
            ...
            children: [
              ...
              if (_pronunciationAttempts >= 2 &&
                  fathaTapRepeatAssetFor(_letter) != null)
                Flexible(
                  child: TextButton.icon(
                    key: const ValueKey('checkpoint-hear-example'),
                    onPressed: _hearPronunciation,
