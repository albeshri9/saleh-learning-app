...
import '../../../services/audio/fatha_reading_demonstration.dart';
...
class _PronunciationSceneState extends ConsumerState<PronunciationScene> {
  ...
  bool _feedbackComplete = false;

  String? get _exampleAsset =>
      widget.scene.data['letterTapAudio'] as String? ??
      fathaTapRepeatAssetFor(widget.scene.data['letter'] as String? ?? '');
  ...
  Future<void> _hearExample() async {
    final asset = _exampleAsset;
    if (asset == null) return;
    final request = ++_request;
    widget.api.channel.interruptScript();
    setState(() => _state = _MicState.tryAgain);
    await _audio.stop();
  ...
  Future<void> _listen() async {
    ...
    try {
      ...
      if (mounted && request == _request) {
        setState(() => _feedbackComplete = true);
      }
    }
  }
          ...
          child: SizedBox(
            ...
            child: Column(
              children: [
                ...
                SizedBox(
                  key: const ValueKey('pronunciation-footer'),
                  height: compact ? 102 : 118,
                  child: Column(
                    children: [
                      SizedBox(
                        key: const ValueKey('pronunciation-feedback'),
                      ...
                      Expanded(
                        child: Row(
                          ...
                          children: [
                            ...
                            if (_state != _MicState.correct &&
                                _completedAttempts >= 2 &&
                                _exampleAsset != null)
                              Flexible(
                                child: TextButton.icon(
                                  key: const ValueKey(
                                      'pronunciation-hear-example'),
                                  onPressed: _hearExample,
                                  icon: const Icon(Icons.volume_up_rounded,
                                      size: 20),
                                  label: const Text('استمعوا للنطق',
                                      textAlign: TextAlign.center),
                                ),
                              ),
                            if ((_state == _MicState.correct &&
                                ...
                                (_state != _MicState.correct &&
                                    _completedAttempts >= 2))
                              LessonActionButton(
                                label: _state == _MicState.correct
                                    ? 'متابعة'
                                    : 'تخطي',
