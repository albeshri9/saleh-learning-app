...
class _ReadingAssessmentState extends ConsumerState<ReadingAssessmentScene> {
  ...
  late final FathaReadingDemonstration _demonstration;
  ...
  void initState() {
    ...
    _demonstration = FathaReadingDemonstration(_audio);
  ...
  Future<void> _hearReading() async {
    if (_result || _correct || _attempts < 2) return;
    final request = ++_request;
    _demonstration.cancel();
    widget.api.channel.interruptScript();
    InteractionAudio.stopCelebration();
    setState(() {
      _listening = false;
      _modelPlaying = true;
      _feedback = 'استمعوا ثم حاولوا مرة أخرى';
    });
    await _speech.dispose();
    if (!mounted || request != _request) return;
    try {
      await _demonstration.play(List.of(_current));
    } finally {
      if (mounted && request == _request) {
        setState(() => _modelPlaying = false);
      }
    }
  }
        ...
        SizedBox(
          ...
          child: Row(
              ...
              children: [
                ...
                if (!_result && !_correct && _attempts >= 2)
                  Flexible(
                    child: TextButton.icon(
                      key: const ValueKey('reading-hear-example'),
                      onPressed: _modelPlaying ? null : _hearReading,
                      icon: const Icon(Icons.volume_up_rounded, size: 20),
                      label: const Text('استمعوا للقراءة',
                          textAlign: TextAlign.center),
                    ),
                  ),
                if (_result)
