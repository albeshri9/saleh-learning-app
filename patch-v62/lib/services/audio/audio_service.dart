...
/// A bounded excerpt of an existing recording, in the original asset clock.
/// Keeping these boundaries explicit avoids replaying the next repetition or
/// question when the child asks to hear a joined reading example.
class AudioClip {
  const AudioClip({
...
/// Optional capability: a service without clipping must not substitute a full
/// three-repeat recording for one phoneme in a two/three-letter example.
abstract interface class AudioClipService {
...
class AssetAudioService implements AudioService, AudioClock, AudioClipService {
  AssetAudioService() {
  ...
  @override
  Future<void> play(String assetPath) => _playAsset(assetPath);

  @override
  Future<void> playClip(AudioClip clip) =>
      _playAsset(clip.assetPath, clip: clip);

  Future<void> _playAsset(String assetPath, {AudioClip? clip}) async {
    ...
    try {
      ...
      if (requestId != _requestId) return;
      if (clip != null) {
        await _player.setClip(start: clip.start, end: clip.end);
        if (requestId != _requestId) return;
      }
      await _player.play();
    } catch (error) {
