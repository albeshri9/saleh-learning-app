...
void main() {
  ...
  test('المحتوى يربط الترحيب والتتبع والنهاية بالحركات والأصوات المعتمدة', () {
    ...
    expect(
      success.lines.single.events.first.params['name'],
      'celebrateThenWave',
    );
    expect(success.lines.single.events.last.params['name'], 'farewell');
    expect(success.lines.single.events.last.at,
        const Duration(milliseconds: 8650));
  });
  ...
  test('التحقق الصوتي يقبل صيغ تفريغ صوت آ الواقعية', () {
    ...
    expect(speechMatchesExpected('آه', 'آ'), isFalse);
  ...
  testWidgets('الكتابة الحرة ترفض الخربشة وتقبل أَ من ضغطة واحدة',
    ...
    await tester.pump();
    expect(
      find.text('ممتاز! أحسنتم كتابة الحرف'),
      findsOneWidget,
    );
    expect(audio.lastAsset, 'assets/audio/v46/free_praise.mp3');
    expect(completed, 0);
    audio.finish();
  ...
  testWidgets('التقويم شفهي ويجعل صالح يفكر ولا ينتظر صوت النجاح',
    ...
    await tester.pump();
    await tester.tap(
        find.byWidgetPredicate((w) => w is LetterGlyph && w.letter == 'أَ'));
    await tester.pump();
