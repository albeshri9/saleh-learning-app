...
class _Audio implements AudioService, AudioClipService {
  _Audio(this.events);
  ...
  @override
  Future<void> playClip(AudioClip clip) {
    clips.add(clip);
    return play(clip.assetPath);
  }
...
void main() {
  for (final letters in [
    ...
    testWidgets(
        'model appears after two failures and reads $letters once in order',
        (tester) async {
      final harness = _Harness();
      for (var i = 0; i < 2; i++) {
        harness.speech.responses.add((_) async =>
            const SpeechResult(correct: false, recognizedWords: 'خطأ'));
      }
      await harness.mount(tester, items: [letters], size: const Size(320, 220));
      final model = find.byKey(const ValueKey('reading-hear-example'));
      ...
      expect(harness.answers, [false, false]);
      await tester.tap(model);
      await tester.pump();
      await tester.pump();
      for (var index = 0; index < letters.length; index++) {
        expect(harness.audio.clips, hasLength(index + 1));
        final expected = fathaModelClipFor(letters[index])!;
        final clip = harness.audio.clips[index];
        expect(clip.assetPath, expected.assetPath);
        expect(clip.start, expected.start);
        expect(clip.end, expected.end);
        harness.audio.finishClip();
        await tester.pump();
      }
      expect(harness.audio.clips, hasLength(letters.length));
      expect(harness.answers, [false, false],
          reason: 'Hearing never scores an answer');
      expect(harness.attempts, 2);
      expect(harness.audio.playing.value, isFalse);
      final board =
          tester.getRect(find.byKey(const ValueKey('test-reading-board')));
      _inside(tester.getRect(model), board, 'model button');
      expect(tester.takeException(), isNull);
    });
  }

  for (final interrupt in ['mic', 'skip', 'navigation']) {
    testWidgets('$interrupt cancels the remaining example phonemes',
        (tester) async {
      final harness = _Harness();
      for (var i = 0; i < 2; i++) {
        harness.speech.responses.add((_) async =>
      ...
      await tester.pump(const Duration(milliseconds: 200));
      await _tapMic(tester);
      await tester.tap(find.byKey(const ValueKey('reading-hear-example')));
      await tester.pump();
      await tester.pump();
      expect(harness.audio.clips, hasLength(1));
      final before = harness.events.length;
      if (interrupt == 'mic') {
        await _tapMic(tester);
        final events = harness.events.skip(before).toList();
        expect(events.indexOf('stop-audio'),
            lessThan(events.indexOf('listen:قَ شَ')));
      } else if (interrupt == 'skip') {
  ...
  for (final size in [const Size(320, 220), const Size(600, 340)]) {
    for (final letters in [
      ...
      testWidgets('fits ${letters.length} RTL glyphs and feedback at $size',
        ...
        final successStyle = tester.widget<Text>(find.text('أحسنتم يا أبطال.'));
