...
void main() {
  for (final reading in [
    ...
    testWidgets('accepts matching interim ${reading.first} immediately',
        (tester) async {
      final recognizer = _Recognizer()..stopGate = Completer<void>();
      final service = DeviceSpeechService(recognizer: recognizer);
      SpeechResult? completed;
      final pending =
          service.listenFor(reading.first).then((value) => completed = value);
      await tester.pump();
      recognizer.result(reading.last);
      await tester.pump();
      expect(completed?.correct, isTrue,
          reason: 'Neither finalResult nor native stop must delay success');
      expect(recognizer.stopCalls, 1);
      expect(recognizer.listening, isFalse);
      recognizer.result('كلام آخر', finalResult: true);
      await tester.pump();
      expect(completed?.recognizedWords, reading.last);
  ...
  testWidgets('does not combine unrelated partial snapshots into a pair',
      (tester) async {
    final recognizer = _Recognizer();
    final service = DeviceSpeechService(recognizer: recognizer);
    SpeechResult? completed;
    final pending = service.listenFor('قَ شَ').then((v) => completed = v);
    await tester.pump();
    for (final transcript in ['قاء', 'شاء', 'كا شا', 'شا قا']) {
      recognizer.result(transcript);
      await tester.pump(const Duration(seconds: 2));
      expect(completed, isNull, reason: transcript);
      expect(recognizer.stopCalls, 0);
    }
    recognizer.result('قاشا');
    await tester.pump();
    expect(completed?.correct, isTrue);
    await pending;
    await service.dispose();
  ...
  testWidgets('an ambiguous raa does not finish before the separate alif',
      (tester) async {
    final recognizer = _Recognizer();
    final service = DeviceSpeechService(recognizer: recognizer);
    SpeechResult? completed;
    final pending = service.listenFor('رَ أَ').then((v) => completed = v);
    await tester.pump();
    for (final transcript in ['را', 'راء', 'رَا']) {
      recognizer.result(transcript);
      await tester.pump(const Duration(seconds: 2));
      expect(completed, isNull, reason: transcript);
    }
    recognizer.result('را آ');
    await tester.pump();
    expect(completed?.correct, isTrue);
    await pending;
    await service.dispose();
  ...
  testWidgets('next attempt waits for previous microphone cleanup only',
      (tester) async {
    final recognizer = _Recognizer()..stopGate = Completer<void>();
    final service = DeviceSpeechService(recognizer: recognizer);
    final first = service.listenFor('قَ شَ');
    await tester.pump();
    recognizer.result('قا شا');
    await tester.pump();
    expect((await first).correct, isTrue);
    final second = service.listenFor('بَ حَ');
    await tester.pump();
    expect(recognizer.listenCalls, 1);
    recognizer.stopGate!.complete();
    recognizer.stopGate = null;
    await tester.pump();
    expect(recognizer.listenCalls, 2);
    recognizer.result('با حا');
    await tester.pump();
    expect((await second).correct, isTrue);
    await service.dispose();
  });
