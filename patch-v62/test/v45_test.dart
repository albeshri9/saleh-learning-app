...
void main() {
  ...
  test('Ja ASR spelling variants and repetitions do not accept unrelated words',
      () {
    for (final text in ['جا', 'جاء', 'جَا', 'جاء، جاء', 'الجاء']) {
      expect(speechMatchesExpected(text, 'جَا'), true, reason: text);
    }
    for (final text in ['جيم', 'جمل', 'تاء', 'حاء', 'هذا جمل', 'جاء جمل', '']) {
  ...
  testWidgets('guided writing is mandatory and exposes no skip action',
      (tester) async {
    final channel = SceneChannel();
    var skipped = 0;
    final api = SceneApi(
    ...
    expect(find.text('تخطي الشرح'), findsNothing);
    expect(find.text('تخطي'), findsNothing);
    expect(skipped, 0);
    await tester.pumpWidget(const SizedBox());
    await tester.pump(const Duration(seconds: 1));
